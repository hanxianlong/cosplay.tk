...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
	...
	function add_dmsguestbook() {
		...
		$maxRole ='level_10';// max($Role1,$Role2,$Role3);
                $Role1=$Role2= $Role3= 'level_7';
		...
		add_menu_page(__('Options', 'dmsguestbook'), '预约单', $maxRole,
		'Entries', 'dmsguestbook2_meta_description_option_page', '../wp-content/plugins/dmsguestbook/img/guestbook.png');
...
<?php
	...
	if($_REQUEST['tinymce']!=1 && $_REQUEST['htmleditor']!=1) {
		...
		<th style='padding:0px 5px 0px 5px; width:20px;'>" . __("ID", "dmsguestbook") . "</th>
	 	<th style='padding:0px 5px 0px 5px; width:100px;'>预订人信息</th>
	 	<th style='padding:0px 5px 0px 5px; width:300px;'>留言内容</th>
		<th style='padding:0px 5px 0px 5px; width:200px;'>操作</th>
		</tr>
		</thead>";
	}
			...
			foreach ($query1 as $result) {
				...
				$gbname 	= preg_replace("/[\\\\\"=\(\)\{\}]+/i", "", stripslashes($result->name));
                                $gburl 	= preg_replace("/[\\\\\"=\(\)\{\}]+/i", "", stripslashes($result->url));
		...
		if($_REQUEST['tinymce']!=1 && $_REQUEST['htmleditor']!=1) {
			...
			echo "<td $adminreview_color><b>$concat_name_url</b><br />$email" . "$gbip<br /> $gburl <br/>"  . "$gbadditional2</td>";
